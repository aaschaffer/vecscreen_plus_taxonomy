...
sub add_taxonomy_columns {
	...
	print  $output_filehandle "$local_accession\t";
        ...
        # EPN QUESTION: if $genus_hash{$local_accession} is not defined, then 
        # the resulting output line will have 1 fewer fields. Is that correct?
        # Should $genus_hash{$local_accession} always be defined?
