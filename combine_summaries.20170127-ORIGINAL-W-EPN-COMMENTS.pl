...
while(defined($nextline = <TERMINAL>)) {
    ...
    # EPN QUESTION: Is it possible for there to be more than one line in $input_terminal_file
    # with same accession? If so, don't you want to potentially update %overall_strength_hash 
    # here even if it is defined (i.e. you've previously seen a line in this file for the same accession)
    # In my modified version of the code (current combine_summaries.pl) I have 
    # made this change (that is, the %overall_strength_hash value is potentially updated)
    # because I've put the common code that parses both the TERMINAL and INTERNAL
    # files into a single subroutine, and your code for parsing the INTERNAL matches
    # below was already doing this.
	...
	if ($next_terminal < $num_terminal_lines) {
		...
		if (defined($internal_accession_hash{$terminal_accessions[$next_terminal]})) {
			...
			if (compareStrengths($terminal_strengths[$next_terminal], $internal_strengths[$special_next_internal])) {
                            #EPN: I'm not sure why you don't check to make sure that $terminal_printed[$terminal_next_internal] is not '1'
                            #     here and skip printing if it is, are you really guaranteed that it won't be 1? 
			    ...
			    $next_terminal++;
                            ...
                            #EPN: I'm not sure why you don't check to make sure that $internal_printed[$special_next_internal] is not '1'
                            #     here and skip printing if it is, are you really guaranteed that it won't be 1? 
		    ...
		    while (($next_terminal < $num_terminal_lines) && ($terminal_accessions[$next_terminal] eq $repeated_accession)) {
                      #EPN: I'm not sure why you don't check to make sure that $internal_printed[$next_terminal] is not '1'
                      #     here, are you really guaranteed that it won't be? 
			print OUTPUT "$terminal_lines[$next_terminal]";
			print OUTPUT "\n";
			$terminal_printed[$next_terminal] = 1;
			$num_printed++; 
		    ...
		    while (($special_next_internal < $num_internal_lines) && ($internal_accessions[$special_next_internal] eq $repeated_accession)){
                      #EPN: I'm not sure why you don't check to make sure that $internal_printed[$internal_next_internal] is not '1'
                      #     here and skip printing if it is, are you really guaranteed that it won't be 1? 
			print OUTPUT "$internal_lines[$special_next_internal]";
			print OUTPUT "\n";
			$internal_printed[$special_next_internal] = 1;
